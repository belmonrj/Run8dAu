...
void doit_file(int centid=0, int pionid=0, int kaonid=4)
{
  ...
  // --- kaonpos --- //
  // ----------------- //
  sprintf(filename,"../Spectra/Data/FinalFeed/ptpid_tofw_pos_%d_dis_cent%s.dat",kaonid,centstring);
  fin.open(filename);
  for(int i=0; i<30; i++)
    {
  ...
  fin.close();
  sprintf(filename,"../Spectra/Data/FinalFeed/R5_ptpid_tofw_pos_%d_dis_cent%s.dat",kaonid,centstring);
  fin.open(filename);
  for(int i=0; i<12; i++)
    {
      if(i<6) fin>>tmp>>tmp>>tmp;
  ...
  // --- kaonneg --- //
  // ----------------- //
  sprintf(filename,"../Spectra/Data/FinalFeed/ptpid_tofw_neg_%d_dis_cent%s.dat",kaonid,centstring);
  fin.open(filename);
  for(int i=0; i<30; i++)
    {
  ...
  fin.close();
  sprintf(filename,"../Spectra/Data/FinalFeed/R5_ptpid_tofw_neg_%d_dis_cent%s.dat",kaonid,centstring);
  fin.open(filename);
  for(int i=0; i<12; i++)
    {
      if(i<6) fin>>tmp>>tmp>>tmp;
  ...
  fin1.close();  
  fin2.close();
  sprintf(fin1name,"../Spectra/ppg026/poskaon_%s.dat",centstring);
  sprintf(fin2name,"../Spectra/ppg026/negkaon_%s.dat",centstring);
  fin1.open(fin1name);
  fin2.open(fin2name);
  for(int i=0; i<18; i++)
    {
  ...
  tmg2->SetMinimum(0.0);
  tmg2->GetXaxis()->SetLimits(0.0,4.0);
