void fit_doublegauss_sigmalized_cent_v2(
				       ...
				       )
{
  ...
  char histnamedz[50];
  sprintf(histnamedz,"hh_%s_cent_tofwsdz",type);
  cout<<"name of tofwdz histogram is "<<histnamedz<<endl;

  char histnamedphi[50];
  sprintf(histnamedphi,"hh_%s_cent_tofwsdphi",type);
  ...
  for(int i=0; i<90; i++)
    {
      ...
      hhdz->Fit("fun","R");
      sprintf(figname,"DoubleGauss_SigmaCent/figs/hh_%s_cent_tofwsdz_%s_%d.gif",type,field,i);
      c1->Print(figname);
      foutdz<<cent<<"\t"
	    <<maximum<<"\t"
	    <<fun->GetParameter(1)<<"\t"
      ...
      hhdphi->Fit("fun","R");
      sprintf(figname,"DoubleGauss_SigmaCent/figs/hh_%s_cent_tofwsdphi_%s_%d.gif",type,field,i);
      c1->Print(figname);
      foutdphi<<cent<<"\t"
	      <<maximum<<"\t"
	      <<fun->GetParameter(1)<<"\t"
