...
void doit(int sector=0, int pid=0)
{
  ...
  for(int i=0; i<100; i++)
    {
      temm = h_sim_tofw_pos_pp->GetBinContent(i+1);
      etemm = h_sim_tofw_pos_pp->GetBinError(i+1);
      if(temm>0)
	{
	  value_pospp[i] = 1.0/temm;
	  evalue_pospp[i] = etemm/temm/temm;
	}
      else
	{
	  value_pospp[i] = -9;
      ...
      temm = h_sim_tofw_neg_pp->GetBinContent(i+1);
      etemm = h_sim_tofw_neg_pp->GetBinError(i+1);
      if(temm>0)
	{
	  value_negpp[i] = 1.0/temm;
	  evalue_negpp[i] = etemm/temm/temm;
	}
      else
	{
	  value_negpp[i] = -9;
      ...
      temm = h_sim_tofw_pos_mm->GetBinContent(i+1);
      etemm = h_sim_tofw_pos_mm->GetBinError(i+1);
      if(temm>0)
	{
	  value_posmm[i] = 1.0/temm;
	  evalue_posmm[i] = etemm/temm/temm;
	}
      else
	{
	  value_posmm[i] = -9;
      ...
      temm = h_sim_tofw_neg_mm->GetBinContent(i+1);
      etemm = h_sim_tofw_neg_mm->GetBinError(i+1);
      if(temm>0)
	{
	  value_negmm[i] = 1.0/temm;
	  evalue_negmm[i] = etemm/temm/temm;
	}
      else
	{
	  value_negmm[i] = -9;
  ...
  sprintf(filename,"CorrectionFactors/cf_%s_pospp_%d_dis.dat",sectorstring,pid);
  fout.open(filename);
  for(int i=0; i<100; i++)
    {
      fout<<pt_pospp[i]<<"\t"<<value_pospp[i]<<"\t"<<evalue_pospp[i]<<endl;
    }
  fout.close();
  ...
  sprintf(filename,"CorrectionFactors/cf_%s_negpp_%d_dis.dat",sectorstring,pid);
  fout.open(filename);
  for(int i=0; i<100; i++)
    {
      fout<<pt_negpp[i]<<"\t"<<value_negpp[i]<<"\t"<<evalue_negpp[i]<<endl;
    }
  fout.close();
  ...
  sprintf(filename,"CorrectionFactors/cf_%s_posmm_%d_dis.dat",sectorstring,pid);
  fout.open(filename);
  for(int i=0; i<100; i++)
    {
      fout<<pt_posmm[i]<<"\t"<<value_posmm[i]<<"\t"<<evalue_posmm[i]<<endl;
    }
  fout.close();
  ...
  sprintf(filename,"CorrectionFactors/cf_%s_negmm_%d_dis.dat",sectorstring,pid);
  fout.open(filename);
  for(int i=0; i<100; i++)
    {
      fout<<pt_negmm[i]<<"\t"<<value_negmm[i]<<"\t"<<evalue_negmm[i]<<endl;
    }
  fout.close();
  ...
  if(pid<=1) legpp->AddEntry(tge_pospp,"#pi^{+}","p");
  if(pid<=1) legpp->AddEntry(tge_negpp,"#pi^{-}","p");
  if(pid==2||pid==3) legpp->AddEntry(tge_pospp,"K^{+}","p");
  if(pid==2||pid==3) legpp->AddEntry(tge_negpp,"K^{-}","p");
  if(pid>=4) legpp->AddEntry(tge_pospp,"p","p");
  if(pid>=4) legpp->AddEntry(tge_negpp,"#bar{p}","p");
  legpp->Draw();
  ...
  if(pid<=1) legmm->AddEntry(tge_posmm,"#pi^{+}","p");
  if(pid<=1) legmm->AddEntry(tge_negmm,"#pi^{-}","p");
  if(pid==2||pid==3) legmm->AddEntry(tge_posmm,"K^{+}","p");
  if(pid==2||pid==3) legmm->AddEntry(tge_negmm,"K^{-}","p");
  if(pid>=4) legmm->AddEntry(tge_posmm,"p","p");
  if(pid>=4) legmm->AddEntry(tge_negmm,"#bar{p}","p");
  legmm->Draw();
  ...
  tge_negmm->SetMarkerStyle(25);
  tge_negmm->SetMarkerColor(kBlue);
  tmg_gcf->Add(tge_pospp);
  tmg_gcf->Add(tge_negmm);
  tmg_gcf->Draw("ap");
  ...
  if(pid<=1) leggcf->AddEntry(tge_pospp,"#pi^{+}","p");
  if(pid<=1) leggcf->AddEntry(tge_negmm,"#pi^{-}","p");
  if(pid==2||pid==3) leggcf->AddEntry(tge_pospp,"K^{+}","p");
  if(pid==2||pid==3) leggcf->AddEntry(tge_negmm,"K^{-}","p");
  if(pid>=4) leggcf->AddEntry(tge_pospp,"p","p");
  if(pid>=4) leggcf->AddEntry(tge_negmm,"#bar{p}","p");
  legmm->Draw();
  ...
  tge_negpp->SetMarkerStyle(25);
  tge_negpp->SetMarkerColor(kBlue);
  tmg_bcf->Add(tge_posmm);
  tmg_bcf->Add(tge_negpp);
  tmg_bcf->Draw("ap");
  ...
  if(pid<=1) legbcf->AddEntry(tge_posmm,"#pi^{+}","p");
  if(pid<=1) legbcf->AddEntry(tge_negpp,"#pi^{-}","p");
  if(pid==2||pid==3) legbcf->AddEntry(tge_posmm,"K^{+}","p");
  if(pid==2||pid==3) legbcf->AddEntry(tge_negpp,"K^{-}","p");
  if(pid>=4) legbcf->AddEntry(tge_posmm,"p","p");
  if(pid>=4) legbcf->AddEntry(tge_negpp,"#bar{p}","p");
  legmm->Draw();
