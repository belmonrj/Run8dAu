void draw_resolution_vF()
{
  ...
  ifstream fstrip("striptofw/striptofw_gaus.dat");
  float tmp;
  ...
  tg_strip->GetYaxis()->SetTitle("Timing resolution (ns)");
  tg_strip->GetXaxis()->CenterTitle();
  tg_strip->GetYaxis()->CenterTitle();

  c1->Print("figure_resolution_strip-by-strip.gif");
  c1->Print("figure_resolution_strip-by-strip.eps");
  ...
  for(int i=0; i<848; i++)
      ...
      if(eresR[i]>0.1)
	{
      ...
      if(resR[i]>0.110) cout<<"runindex "<<i<<endl;
      //if(i%50==0) cout<<i<<"\t"<<res[i]<<"\t"<<eres[i]<<endl;
    }
  ...
  tg_run->GetYaxis()->SetTitle("Timing resolution (ns)");
  tg_run->GetXaxis()->CenterTitle();
  tg_run->GetYaxis()->CenterTitle();

  c2->Print("figure_resolution_run-by-run.gif");
  c2->Print("figure_resolution_run-by-run.eps");
