void fitm2_tripleG4_R5()
{
  ...
  for(int INDEX=0; INDEX<10; INDEX++)
      ...
      if(i>=40&&i<50)
	{
      ...
      superfun->SetParLimits(2,0.99*sigmapi->Eval(mom),1.01*sigmapi->Eval(mom)); // sigma
      //superfun->SetParLimits(3,0.9*funkaon->GetParameter(0),1.1*funkaon->GetParameter(0));
      superfun->SetParLimits(4,0.9*m2ka,1.00*m2ka);
      superfun->SetParLimits(5,0.99*sigmaka->Eval(mom),1.01*sigmaka->Eval(mom)); // sigma
      //superfun->SetParLimits(6,0.9*funprot->GetParameter(0),1.1*funprot->GetParameter(0));
      superfun->SetParLimits(7,1.00*m2pr,1.1*m2pr);
      superfun->SetParLimits(8,0.99*sigmapr->Eval(mom),1.01*sigmapr->Eval(mom)); // sigma
      ...
      funkaonA->Draw("same");
      funprotA->Draw("same");
      sprintf(pionfigname,"m2pT_figs/m2pT_both_tripleG_method4R5_%d.gif",i);
      c1->Print(pionfigname);
      sprintf(pionfigname,"m2pT_figs/m2pT_both_tripleG_method4R5_%d.eps",i);
      c1->Print(pionfigname);
