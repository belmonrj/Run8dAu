void fitm2_pika5()
{
  ...
  for(int i=0; i<40; i++)
      ...
      if(i>=30&&i<40)
	{
      ...
      superfun->SetParLimits(4,0.9*m2ka,1.1*m2ka);
      ...
      // c1->Print(pionfigname);
      if(i<30)help->Fit("superfun","","",-0.2,0.4);
      if(i>=30)help->Fit("superfun","","",-0.4,0.5);
      ...
      funpionA->Draw("same");
      funkaonA->Draw("same");
      sprintf(pionfigname,"m2pT_figs/m2pT_both_pionkaon_method5_%d.gif",i);
      c1->Print(pionfigname);
      sprintf(pionfigname,"m2pT_figs/m2pT_both_pionkaon_method5_%d.eps",i);
      c1->Print(pionfigname);
