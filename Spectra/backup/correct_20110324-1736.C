...
void doit(int sector=0, int pid=0, int centid=0)
{
  ...
  float konstant_pp = nevents_pp*tofw_eff*tofw_adc*dcqual*binwidth_pT*binwidth_cent*mistake;
  float konstant_mm = nevents_mm*tofw_eff*tofw_adc*dcqual*binwidth_pT*binwidth_cent*mistake;
  ...
  for(int i=0; i<100; i++)
    {
      ...
      pT = (pT_A+pT_B)/2.0;
      yield = (yield_A+yield_B)/2.0;
      eyield = sqrt(yield_A**2+yield_B**2);
