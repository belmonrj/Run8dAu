      ...
      for(int i=0;i<n;i++) // loop over events
	{
	  ...
	  for(int itrk=0; itrk<ktree->mNtrack; itrk++)
	      ...
	      if(regular_cuts&&edge&&inner&&zed<0)
		{
	      ...
	      bool dis1 = isPiLow>=-2 && isPiLow<=2 && isKLow<=-2 && ep>0.2;
	      bool dis2 = isPiLow>=-2 && isPiLow<=2 && isKLow<=-2 && fabs(zed)>40;
	      bool dis3 = isPiLow>=-2 && isPiLow<=2 && isKLow<=-2 && fabs(zed)<40;
	      //kaons
	      ...
	      bool dis7 = isPLow>=-2 && isPLow<=2 && isKLow>=2 && ep>0.2;
	      bool dis8 = isPLow>=-2 && isPLow<=2 && isKLow>=2 && fabs(zed)>40;
	      bool dis9 = isPLow>=-2 && isPLow<=2 && isKLow>=2 && fabs(zed)<40;
...
bool GoodDCH(int run, float zed, float alpha, float phi)
{
  ...
  return true; // render ineffective for now
