void draw_shift_vF()
{
  ...
  ifstream fstrip1("striptofw/striptofw_gaus.dat");
  ...
  for(int i=0; i<512; i++)
    {
      fstrip1>>stripid[i]>>tmp>>shift[i]>>eshift[i]>>tmp>>tmp;
      zero[i] = 0;
      if(eshift[i]>0.05)
	{
	  shift[i] = -9;
  ...
  cout<<howmany<<" strip skipped"<<endl;
  ...
  tg_strip->Draw("AP");
  tg_strip->Fit("fun","R");
  tg_strip->SetMaximum(0.1);
  tg_strip->SetMinimum(-0.1);
  ...
  c1->Print("figure_shift_FINAL_strip-by-strip.gif");
  c1->Print("figure_shift_FINAL_strip-by-strip.eps");
  ...
  ifstream frun("run/run_gaus.dat");
  float tmpR;
  ...
  for(int i=0; i<848; i++)
    {
      frun>>runid[i]>>tmpR>>shiftR[i]>>eshiftR[i]>>tmpR>>tmpR;
      zeroR[i] = 0;
      if(eshiftR[i]>1.0)
	{
	  shiftR[i] = -9;
      ...
      if(fabs(shiftR[i])>0.01) cout<<"runindex "<<i<<endl;
  ...
  c2->Print("figure_shift_FINAL_run-by-run.gif");
  c2->Print("figure_shift_FINAL_run-by-run.eps");
